...
void Calc_Imaginary(int a, int b, int c) {
	double D, root_D, root_ac1, root_ac2, root_im1, root_im2;
	D = -Calc_D(a, b, c);
	root_D = sqrt(D);
	root_ac1 = -b / (double)(2 * a);
	root_ac2 = -b / (double)(2 * a);
	root_im1 = root_D / (double)(2 * a);
	root_im2 = root_D / (double)(2 * a);
	printf("The root are %.3f+%.3fi, %.3f-%.3fi", root_ac1, root_im1, root_ac2, root_im2);
}
