...
void Tactical_set_11(char b[][3], int m);
void Tactical_set_12(char b[][3], int m);
void diagonal_coner(char b[][3], int *m);
int diagonal_coner_count(char b[][3]);
...
void Attack_Mode(char b[][3],int t_count, int *m) {
	...
	if (t_count == 1) {//만약 자신이 첫 수 라면 자신이 이길 수 있는 유리한 수를 둠
		...
		else if ((diagonal_coner_count(b) == 2 )&&(b[1][0] == 'O' || b[0][1] == 'O' || b[1][2] == 'O' || b[2][1] == 'O')) {
			Tactical_set_3(b, m);
		}
		else if (diagonal_coner_count(b) == 2) {
			diagonal_coner(b, m);
			return;
		}
	}
