/*
  ...
  -알고리즘 fix = 새로운 방어전략과 공격전략이 생겼습니다(ver.1.4.5)
  -전술_5가 업데이트 되었습니다.(ver 1.4.6)
  -방어모드가 업데이트되었습니다. (ver. 1.4.7)
*/
#include<stdio.h>
#include<stdlib.h>
#include<time.h>
...
void human_move(char b[][3]) {
	...
	while (1) {
		scanf("%d%d", &i, &j);
		if (i < 0 || i>2 || j < 0 || j>2) { //잘못된것에 두면 다시 두게 만듬
...
// < 전술법 set_1 > 유리한 수인 가운데 혹은 모서리에 두는 것에 랜덤성 부여 
void Tactical_set_1(char b[][3], int *m) {
...
//< 전술법 set_2 > 선공-컴퓨터가 가운데 두었을때 다음 차례 상대가 두는 위치에 따라 승리 판별
void Tactical_set_2(char b[][3], int *m) {
...
//< 전술법 set_3 >  모서리에 두되 위,아래,옆에 상대 돌이 없는 경우에 둔다.
void Tactical_set_3(char b[][3], int *m) {
	while (1) {
...
// < 전술법 set_4 > 선공-컴퓨터가 모서리에 두었을때 이전 상대가 둔 수에 따라 승리전략 선택
void Tactical_set_4(char b[][3], int *m) {
	int st;
...
// < 전술법 set_5 > (선공)때 플레이어가 가운데를 막았다면 건너 모서리에 둔다.
void Tactical_set_5(char b[][3], int *m) {
	if (b[0][0] == 'X') {
		b[2][2] = 'X';
		*m = 1;
...
// < 전술법 set_6 > 대각선 크로스값에서 반대편 코너를 제외한 값에다 둠
void Tactical_set_6(char b[][3], int *m) {
	int st;
	while (1){
		st = rand() % 3;
		if (b[0][0] == 'X' || b[2][2] == 'X') {
			...
			else if (st == 2 && b[2][2] == ' '&&b[0][1] != 'O' && b[1][2] != 'O') {
				b[2][2] = 'X';
				*m = 1;
				break;
			}
