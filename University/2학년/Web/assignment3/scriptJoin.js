...
// 요구 조건3 : [ 입력 방식(1) change ]
function clauseCheck(){
    const radios = document.querySelectorAll('[name="agree"]');
    radios.forEach((radios)=>{
        radios.addEventListener('change', (event)=>{
            Check.state = event.currentTarget.value;
        })
    })
}

// 요구 조건3 : [ 입력 방식(2) keydown ]
function RegisterclauseLook(){
    const textarea = document.querySelectorAll('.registerForm');
    textarea.forEach((target)=>{
        target.addEventListener('keydown',()=>{
            if(!Check.state){
                alert("먼저 약관 동의에 체크 하세요");
            }
        });
    })
}
...
// 
function IdRequirements(){
    const textarea = document.querySelector('#inputId');
    const warnText = document.querySelector('#IdRecText');

    textarea.addEventListener('keyup',(event)=>{
        const texting = event.target.value
        AuthForm.name = texting;
        if(texting.length > 2){
            warnText.style.color = "green"
            warnText.textContent = " 사용 가능합니다."
            AuthFormClear.id = true;
        }else{
            warnText.style.color = "red"
            warnText.textContent = " Id가 너무 짧습니다."
            AuthFormClear.name = false;
        }
    });
}
...
function IdRequirements(){
    ...
    textarea.addEventListener('keyup',(event)=>{
        let texting = event.target.value
        AuthForm.id = texting;
        if(texting.length > 2){
            warnText.style.color = "green"
            warnText.textContent = " 사용 가능합니다."
            ...
            warnText.style.color = "red"
            warnText.textContent = " Id가 너무 짧습니다."
            AuthFormClear.id = false;
        }
    });
}


// 요구조건 1번: [ contents 변경, 스타일 변경 ]
// 요구조건 3번: [ 입력 방식(3) keyup ]
function passwordRequirements(){
    const pattern1 = /[0-9]/; //숫자
    const pattern2 = /[a-zA-Z]/; //영어
    const textarea = document.querySelector('#inputPasswd');
    const warnText = document.querySelector('#paswRecText');

    function checkPasswd(texting){
        if(texting.length == 0){
            warnText.textContent = ""
            AuthFormClear.passwd = false;
        }else if(texting.length < 6){
            warnText.style.color = "red"
            warnText.textContent = " 비밀번호가 너무 짧습니다. 6글자 이상"
            AuthFormClear.passwd = false;
            
        }else if(pattern1.test(texting) && pattern2.test(texting)){
            warnText.style.color = "green"
            warnText.textContent = " 사용가능"
            AuthFormClear.passwd = true;
        }else{
            warnText.style.color = "red"
            warnText.textContent = " 형식이 맞지 않습니다. 영어, 숫자 조합"
            AuthFormClear.passwd = false;
        }
    }
    ...
    textarea.addEventListener('keyup',(event)=>{
        let texting = event.target.value
        AuthForm.passwd = texting;
        checkPasswd(texting);
    });
}


function rePasswordRequirements(){
    const textarea = document.querySelector('#inputRePasswd');
    const warnText = document.querySelector('#paswReRecText');

    textarea.addEventListener('keyup',(event)=>{
        let texting = event.target.value
        if(AuthForm.passwd == texting){
            warnText.style.color = "green"
            warnText.textContent = " 일치"
            AuthFormClear.repasswd = true;
        }else{
            warnText.style.color = "red"
            warnText.textContent = " 비밀번호가 다릅니다"
            AuthFormClear.repasswd = false;
        }
    });
}

// 요구조건 3번: [ 입력 방식(4) keypress ]
function emailRequirements(){
    const inputTag = document.querySelector('#inputEmail');
    const warnText = document.querySelector('#emailRecText');
    ...
    inputTag.addEventListener('keypress', (event) => {
        const texting = event.currentTarget.value
        AuthForm.email = texting;
        
        if (isEmail(texting)) {
            warnText.style.color = 'green'
            warnText.textContent = `이메일 형식입니다.`
            AuthFormClear.email = true;
        } else {
            warnText.style.color = 'red'
            warnText.textContent = `이메일 형식이 아닙니다.`
            AuthFormClear.email = false;
        }
    })
}

function JoinSubmit(){
    const target = document.querySelector('#joinSubmit');


    const isGoodFomat = () => {
        const a = AuthFormClear;
        console.log(a.email, a.id, a.passwd, a.repasswd)
        return (a.email && a.id && a.passwd && a.repasswd);
    }
...
// 요구조건 1번: [ contents 변경 ]
function clauseText(){
    const side = document.querySelector('.readTextBox');
    side.textContent = clauseContent();
}


function clauseContent(){
    const COMPANY_NAME = '띵지 스터디 카페'
    const text = `제 1 장 총 칙
    
    제 1 조 (목적)
    이 약관은 ${COMPANY_NAME}(이하 "사이트")에서 제공하는 인터넷서비스(이하 "서비스")의 이용 조건 및 절차에 관한 기본적인 사항을 규정함을 목적으로 합니다.
